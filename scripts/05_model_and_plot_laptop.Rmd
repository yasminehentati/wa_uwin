...
colnames(corr) <- c("Toxic release concentration", "Distance to Superfund", 
                    "PM2.5 concentration", "Percent units with lead", 
                    "Distance to hazardous waste sites",
                    "Population density", "Diesel exhaust emissions", "Wastewater discharge",
                    "Natural land cover", "Median income")

rownames(corr) <- c("Toxic release concentration", "Distance to Superfund", 
                    "PM2.5 concentration", "Percent units with lead", "Distance to hazardous waste sites",
                    "Population density", "Diesel exhaust emissions", "Wastewater discharge",
                    "Natural land cover", "Median income")


corrplot(corr, method = 'color',  type = 'lower', order = 'hclust', tl.col = 'black',
...
colnames(corr) <- c("Natural land cover", "Median income",
                    "Population density", 
                    "Env. exposures", "Env. effects")

rownames(corr) <- c("Natural land cover", "Median income",
                    "Population density", 
                    "Env. exposures", "Env. effects")
...
colnames(B) <- c("city", "Natural land cover", 
                "Median income",
                "Population density",
                "Percent units with lead",
                "PM2.5 concentration",
                "Distance to hazardous waste sites",
                "Distance to Superfund",
                "Toxic release concentrations",
                "Wastewater discharge",
                "Diesel exhaust emissions")

B <- split(B[,1:11], B$city)
...
for (group_name in names(B)) {
  ...
  corrplot(correlations, method = 'color',  type = 'lower', order = 'hclust', tl.col = 'black',
         cl.ratio = 0.2, tl.srt = 45, 
         col = COL1('YlGn'), cl.pos = 'b', addgrid.col = 'white', addCoef.col = 'grey50', title=group_name)
}
...
coymodboth <- glmmTMB(Coyote ~ city + 
                 #     scale(prop_veg_avg) + 
                    scale(natural) + 
...
coymodbase <- glmmTMB(Coyote ~ city + scale(natural) + 
...
coymodint <- glmmTMB(Coyote ~ city * scale(natural) + city * scale(EXPOSURE) + city * scale(EFFECT)  + 
...
coymodsea <- glmmTMB(Coyote ~ scale(natural) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                data = all_dat_sewa, 
                offset = log(days.active),
...
coymodtac <- glmmTMB(Coyote ~ scale(natural) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                data = all_dat_tawa, 
                offset = log(days.active),
...
sjPlot::plot_model(coymod2, title = "Coyotes",
                             rm.terms = "season [Fall20,
                            Spring19, Spring20, Summer19, Summer20,
                            Winter19,Winter20]", 
             #                axis.title = "",
...
# Grouped exposures/effects model
racmodboth <- glmmTMB(Raccoon ~ city + scale(natural) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                   data = all_dat, 
                   offset = log(days.active),
...
# interaction grouped model 
racmodint <- glmmTMB(Raccoon ~ city * scale(natural) + city * scale(EXPOSURE) + city * scale(EFFECT)  + 
...
racmodsea <- glmmTMB(Raccoon ~ scale(natural) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                    data = all_dat_sewa, 
                    offset = log(days.active),
...
racmodtac <- glmmTMB(Raccoon ~ scale(natural) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                    data = all_dat_tawa, 
                    offset = log(days.active),
...
# Grouped exposures/effects model
oppmodboth <- glmmTMB(Virginia_opossum ~ city + scale(natural) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                   data = all_dat, 
                   offset = log(days.active),
...
# interaction grouped model 
oppmodint <- glmmTMB(Virginia_opossum ~ city * scale(natural) + city * scale(EXPOSURE) + city * scale(EFFECT)  + 
                     city * scale(med_income) + season + scale(pop_density)+ ( 1 | site), 
                data = all_dat, 
                offset = log(days.active),
                family = "nbinom2",
...
oppmodsea <- glmmTMB(Virginia_opossum ~ scale(natural) + scale(EXPOSURE) + scale(EFFECT)  + 
                         scale(med_income) + season + scale(pop_density)+ ( 1 | site), 
                    data = all_dat_sewa, 
                    offset = log(days.active),
                    family = "nbinom2")

oppmodtac <- glmmTMB(Virginia_opossum ~ scale(natural) + scale(EXPOSURE) + scale(EFFECT)  + 
                      scale(med_income) + season + scale(pop_density)+ ( 1 | site), 
                    data = all_dat_tawa, 
                    offset = log(days.active),
                    family = "nbinom2")
...
oppmod3a <- glmmTMB(Virginia_opossum ~ scale(prop_veg_avg) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                    data = all_dat_sewa, 
                    offset = log(days.active),
...
oppmod3b <- glmmTMB(Virginia_opossum ~ scale(prop_veg_avg) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                    data = all_dat_tawa, 
                    offset = log(days.active),
...
# Grouped exposures/effects model
simpmodboth <- glmmTMB(simpson.di ~ city + scale(natural) + scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) +
...
# interaction grouped model
simpmodint <- glmmTMB(simpson.di ~ city * scale(natural) + city * scale(EXPOSURE) + city * scale(EFFECT)  + 
...
simpmodsea <- glmmTMB(simpson.di ~ scale(natural) + scale(EXPOSURE) + scale(EFFECT)  + 
                         scale(med_income) + ( 1 | site), 
                    data = all_dat_sewa_veg, 
                    family = "gaussian")

simpmodtac <- glmmTMB(simpson.di ~ scale(natural) + scale(EXPOSURE) + scale(EFFECT)  + 
                      scale(med_income) + ( 1 | site), 
...
tab_model(coymodboth, racmodboth, oppmodboth, 
          collapse.ci = TRUE, 
          p.style     = "numeric_stars",
          rm.terms = c("season [Fall20,
                            Spring19, Spring20, Summer19, Summer20,
...
# table of species grouped models -- seattle *** 
tab_model(coymodsea, racmodsea, oppmodsea, 
          collapse.ci = TRUE, 
          p.style     = "numeric_stars",
          rm.terms = c("season [Fall20,
                            Spring19, Spring20, Summer19, Summer20,
...
# table of species city based models
tab_model(coymod3a, racmod3a, oppmod3a, 
          collapse.ci = TRUE, 
          p.style     = "numeric_stars",
          rm.terms = c("season [Fall20,
                            Spring19, Spring20, Summer19, Summer20,
...
tab_model(coymod3b, racmod3b, oppmod3b, 
          collapse.ci = TRUE, 
          p.style     = "numeric_stars",
          rm.terms = c("season [Fall20,
                            Spring19, Spring20, Summer19, Summer20,
...
plot_models(coymodglobal, racmodglobal, oppmodglobal, title = "Seattle + Tacoma", show.values = T, grid = TRUE,
...
#    scale_y_log10(limits = c(-0.1, 100)) + 
  ...
  scale_x_discrete(labels=c("City", "Average NDVI", "Average median income", "Percent units with lead",
                            ...
                            "Annual Diesel NOx emissions", "Population density"), limits = rev)
...
plot_models(coymodboth, racmodboth, oppmodboth, title = "Both cities", show.values = T, grid = TRUE, vline.color = "gray",
             grid.breaks = c(.1,1,10),
          p.shape     = "FALSE",
         rm.terms = c("season [
                            Winter19,Spring19, Summer19, 
