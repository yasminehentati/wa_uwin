...
p_load(dplyr,here,sf,mapview,remotes,tidycensus,
...
keep_columns <- function(df) {
  df %>%  # Keep city, site, and all other columns
    { if ("season" %in% colnames(.)) select(., -season) else . } %>%  
    { if ("species" %in% colnames(.)) select(., -species) else . } %>%   
    { if ("count" %in% colnames(.)) select(., -count) else . } %>%  
    { if ("dys_ctv" %in% colnames(.)) select(., -dys_ctv) else . }   
}
...
pollutants_sewa <- cleaned_dfs_sewa %>% reduce(full_join, by = c("site")) 
pollutants_sewa
#create percentile for each metric. we'll use this to create the pollution burden score later
...
# combine data by city/site and remove tawa 
pollutants_tawa <- cleaned_dfs_tawa %>% reduce(full_join, by = c("site")) 
