...
wastewater <- read_csv(here("data", "env_health_data", "Diesel_Emission_Levels_of_NOx_(Annual_Tons_Km2).csv"),
...
diesel_dat <- data.frame(
  sites@data,
  "Annual Tons Km2 Diesel NOx" = rep(NA, nrow(sites@data))
)
...
for(i in 1:length(sites)){
  ...
  diesel_dat$Average_PWDIS[i] <- terra::extract(diesel, buff, fun = mean, na.rm = TRUE)
...
diesel_dat <- diesel_dat %>%
  dplyr::select(c(site, city, Average_Diesel_NOx))
...
# for some reason one site copied over multiple times so we'll delete repeats

pollutants <- pollutants[-(62:188),]
...
pollutants <- pollutants %>%
  ...
  mutate(DIESEL_PCT = ntile(pollutants$Average_Diesel_NOx, 100)) 
