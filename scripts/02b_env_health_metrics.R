...
# first list data frames together 
listdfs <- list(lead_dat, pm25_dat, hazwaste_dat, traffic_dat, pnpl_dat,toxic_dat, water_dat)
...
#first we need to average the exposures (pm25, heavytraffic, toxic releases) and environmental effects (cleanupsites, gw threat, and hazards for us)

pollutants <- pollutants %>%
  mutate(EXPOSURE = (PM25_PCT + TRAFFIC_PCT +  TOXIC_PCT) / 3) %>%
