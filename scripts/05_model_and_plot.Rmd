...
#   clean_names() # make spaces underscores %>% 
  rename_with(make_names, c(1:20)) # make spaces underscores for spp names (without janitor)

colnames(all_dat)

## adjust days active based on mason's manual ifno 

# strike P-GW1 Summer 19 
all_dat <- all_dat %>%
  filter(!(site == "P-GW1" & season == "Summer19"))

#  P-GW1 Winter 19 = 29 days
#  G-TNC1 Winter 19 = 29 days
all_dat <- all_dat %>% 
   mutate(site = as.character(site)) %>% 
 mutate(days.active = case_when(site == "P-GW1" | site == "G-TNC1" & 
                               season == "Winter19" ~ 29,
                               TRUE ~ days.active))

# 3. P-OV1 Winter 19 = 28 days
# 4. G-WLP1 Winter 19 = 28 days
all_dat <- all_dat %>% 
   mutate(site = as.character(site)) %>% 
 mutate(days.active = case_when(site == "P-OV1" | site == "G-WLP1" & 
                               season == "Winter19" ~ 28,
                               TRUE ~ days.active))
...
burden_plot_exp <- all_dat %>% distinct(site, .keep_all = TRUE) %>%
  ggplot(aes(x = EXPOSURE)) +
  geom_histogram(fill = "white", colour = "black") +
  facet_grid(city ~ .)

burden_plot_eff <- all_dat %>% distinct(site, .keep_all = TRUE) %>%
  ggplot(aes(x = EFFECT)) +
  geom_histogram(fill = "white", colour = "black") +
  facet_grid(city ~ .)


# correlation between impervius surface and prop veg avg 
...
colnames(corr) <- c("Toxic release concentration", "Distance to Superfund", 
                    ...
                    "Human population density", "Diesel exhaust emissions", "Wastewater discharge",
...
colnames(corr) <- c("Developed land cover", "Median income",
                    "Human population density", 
                    "Env. exposures", "Env. effects")

rownames(corr) <- c("Developed land cover", "Median income",
                    "Human population density", 
                    "Env. exposures", "Env. effects")
...
# Seattle
cordatsewa <- all_dat %>% distinct(site, .keep_all = TRUE) %>% 
  filter(city == "sewa") %>% 
  dplyr::select(c(urban, 
                  med_income,
               #   prop_veg_avg,
                  pop_density,
                  EXPOSURE,
...
corr <- cor(cordatsewa, method = "spearman", use="pairwise.complete.obs")
corrplot(corr, method = "number")
...
# Tacoma
cordattawa <- all_dat %>% distinct(site, .keep_all = TRUE) %>% 
  filter(city == "tawa") %>% 
  dplyr::select(c(develop, 
                  med_income,
               #   prop_veg_avg,
...
corr <- cor(cordattawa, method = "spearman", use="pairwise.complete.obs")
corrplot(corr, method = "number")
...
#### look at correlations -- by city and inidvidual variables
...
colnames(B) <- c("city", "Developed land cover", 
                "Median income",
                "Human population density",
...
write.csv(all_dat, "data_6-4-24-test.csv")
...
coymod3a <- glmmTMB(Coyote ~ scale(develop) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                data = all_dat_sewa, 
                offset = log(days.active),
...
coymod3b <- glmmTMB(Coyote ~ scale(develop) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                data = all_dat_tawa, 
                offset = log(days.active),
...
# Grouped exposures/effects model
racmod2 <- glmmTMB(Raccoon ~ city + scale(prop_veg_avg) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                   data = all_dat, 
                   offset = log(days.active),
...
racmod3a <- glmmTMB(Raccoon ~ scale(prop_veg_avg) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                    data = all_dat_sewa, 
                    offset = log(days.active),
...
racmod3b <- glmmTMB(Raccoon ~ scale(prop_veg_avg) + 
                     scale(EXPOSURE) + scale(EFFECT)  + 
                     scale(med_income) + season + scale(pop_density) + ( 1 | site), 
                    data = all_dat_tawa, 
                    offset = log(days.active),
